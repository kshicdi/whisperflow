...
### 📱 아이폰에서 어디서나 쓰기 (헤드리스 서버)

집에 상시 켜둔 맥(맥미니, 안 쓰는 맥북)을 서버로 만들면, 아이폰 브라우저에서
언제 어디서나 JARVIS에 접속할 수 있습니다 — 음성 입력부터 JARVIS 목소리 응답까지.

```bash
WS_HOST=0.0.0.0 python -m whisperflow.headless
```

전체 구성(launchd 상시구동, Tailscale HTTPS, 목소리 클로닝 TTS)은
**[헤드리스 셋업 가이드](docs/HEADLESS_SETUP.md)** 참고.
